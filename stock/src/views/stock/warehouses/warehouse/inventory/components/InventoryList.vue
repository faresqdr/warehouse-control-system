...
const collection_query = query(collection(db, "warehouses", warehouse_id, "inventory"));
onSnapshot(collection_query, (querySnapshot) => {
    let temp_articles = [];
    querySnapshot.forEach((doc) => {
        temp_articles.push({ id: doc.id, name: doc.data().name, quantity: doc.data().quantity });
    });
    state.articles = temp_articles;
...
<template>
    <div class="hero bg-base-200 pt-14">
        <div class="hero-content text-center">
            <div class="max-w-md">
                <h1 class="text-5xl font-bold">Inventaire</h1>
                <p class="py-6"></p>
            </div>
        </div>
    </div>
    <div class="overflow-x-auto w-full">
        <div class="text-center bg-base-200">
            <button class="btn md:w-96 gap-2">
