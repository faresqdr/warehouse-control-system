...
import AdjustStockHeader from './components/AdjustStockHeader.vue';
...
function switch_step(step_number, data) {
    switch (step_number) {
        ...
        case 3:
            state.step = 4;
            state.loading = true;
            useOperationStore().update_warehouse_inventory(state.warehouse).then(() => {
                state.loading = false;
...
<template>
    <template v-if="state.step == 1">
        <AdjustStockHeader :step_number="1" />
        <IdentifyWarehouse :assigned_step_number="1" @step_validated="switch_step" />
    </template>
    <template v-else-if="state.step == 2">
        <AdjustStockHeader :step_number="2" />
        <ScanArticles :assigned_step_number="2" @previous_step="switch_previous_step" @step_validated="switch_step" />
    </template>
    <template v-else-if="state.step == 3">
        <AdjustStockHeader :step_number="3" />
        <OperationSummary :assigned_step_number="3" @previous_step="switch_previous_step"
            ...
            <div class="flex h-full w-full">
                <div class="grid h-full flex-grow card bg-base-300 rounded-box place-items-center">
                    ...
                    <template v-if="state.error.show">
                        <h1 class="text-2xl md:text-3xl font-semibold text-center text-red-800">
                            {{ state.error?.message }}</h1>
                        <h1 class="text-2xl md:text-3xl font-semibold animate-pulse">Veuillez identifier l'entrepôt</h1>
